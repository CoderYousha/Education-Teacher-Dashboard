...
function UpdateFile() {
     ...
     const updateFile = async () => {
          ...
          let result = await Fetch(host + `/teacher/courses/files/${param.file_id}/update`, 'POST', formData);
     ...
     const getContent = async () => {
          let result = await Fetch(host + `/courses/${param.course_id}/files/${param.file_id}/show`);
                                   ...
                                   <Box className="">
                                        ...
                                        <Box className='overflow-y-scroll h-96 w-1/2 none-view-scroll max-sm:w-full max-sm:text-center'>
                                             <Box className='w-32 h-32 rounded-full mb-2 relative border-2 max-sm:w-24 max-sm:h-24 max-sm:mx-auto'>
                                                  ...
                                                  <img src={content.thumbnail_url} id='image' className='z-30 w-full h-full rounded-full' />
                                             ...
                                             <TextField defaultValue={content.name_en} inputRef={nameEnRef} className="w-4/5" id="standard-basic" label="English Name" variant="standard" slotProps={{ htmlInput: { 'className': 'py-5' } }} />
                                             <Box className='py-3'></Box>
                                             <TextField defaultValue={content.name_ar} inputRef={nameArRef} className="w-4/5" id="standard-basic" label="Arabic Name" variant="standard" slotProps={{ htmlInput: { 'className': 'py-5' } }} />
                                             <Box className='py-3'></Box>
                                             <Button
                                                  component="label"
                                                  role={undefined}
                                             ...
                                             <Box className='py-3'></Box>
                                             <Button onClick={updateFile} variant="contained">
                                                  {
                                                       sendWait ?
